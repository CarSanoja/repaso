...
**Verified deployment:** runtime version 13 handled two successful Telegram explanations on September 14, 2026. The linked retrieval, saved approaches and delivery events are recorded in the [live acceptance record](../../docs/submission/memory-live-check-2026-09-14.md). The School Community Memory observer is a separate local read-only process.
...
The app's invocation body contains an `ok` result or structured error. A valid HTTP response to an unsupported kind proves dispatch and serialization only. A complete acceptance check also needs production services, a real model call and an observed Telegram reply. Those explanation-path checks were performed for version 13; they do not establish every product path or a family pilot. Follow the [deployment and acceptance guide](../README.md).
