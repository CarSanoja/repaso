...
| Every schema parses on the routing that ships | [Conformance on the shipped routing](live-conformance-routing-2026-09-12.json) | 45 live calls at five samples a schema, on the binding the profile chose rather than the one it replaced: 45/45 parsed [0.921, 1.000], 44,435 input and 9,885 output tokens, $0.228573 read from the run's own per-call ledger and reconciled by `scripts/run_cost_report.py` over that file. Five samples is not a rate, a parsed payload is not a good question or a right grade, and 45 probe calls say nothing about sustained load |
