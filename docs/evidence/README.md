...
| The teardown planner selects nothing that exists in the account today | [Dry run](teardown-dry-run.json), [isolation and reversal](../operations/isolation.md) | One read-only run against the authorized account: zero stacks, prefix-bound discovery selects none of the 6 buckets, 9 secrets, 13 log groups, 2 functions or 1 table present, and all 31 are refused by name; no removal was performed, here or anywhere |
