...
| The teardown planner selects nothing that exists in the account today | [Dry run](teardown-dry-run.json) | One read-only run against the authorized account: zero stacks, and none of the 6 buckets, 9 secrets or 13 log groups present is inside the `repaso` prefix; no removal was performed, here or anywhere |
