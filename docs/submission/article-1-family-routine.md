...
The judge experience makes these stages visible without IDs or command-line work. It runs the application graphs in isolated state against a recorded cassette of model responses. Its dates, conversation and assertions are synthetic. A separate real Textract call recognized a new printed Spanish fractions page, a bounded live run confirmed that every configured Bedrock model answers and fills the schema its agent asks for, and the model outputs the cassette replays were recorded from Amazon Bedrock on September 12, 2026. A replay is not a live run, none of it is a family using the product, and no live family benefit is claimed.
