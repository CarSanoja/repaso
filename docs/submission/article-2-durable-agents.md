...
Ten complete adapter simulations now run that chain with Moto emulating AWS, a recorded cassette in place of the models, and substitutions at the AgentCore and Telegram network boundaries. Alternate runs inject an EventBridge publication failure and Telegram failures. Webhook events and queue deliveries are repeated. The tests include enrollment, schedule lifecycle, practice, review, both adult choices and erase. They verify behavior under those substitutions; they do not prove a successful cloud deployment.
