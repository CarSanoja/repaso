...
The [deployment guide](../deploy/README.md) is the current source for release inputs, deploy and teardown commands, and cloud acceptance. Use only the authorized Quanta profile/account. Budget alarms notify; they do not stop all charges. In durable mode, stack removal does not erase pilot data. No stack is claimed deployed from synthesis alone.
