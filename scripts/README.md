...
| `run_demo_scenario.py` | One family's first day, whole, through `handle_channel_message` and the runtime handlers: enrolment on an invite code, the notebook photo, the review that drops 5 of the 8 questions written from it, the capsule, three answers each followed by the harness reading (mastery EMA, next review interval, escalation state), the answer the grader would not sign and the parent's one tap that settles it, and a photocopied guide whose wrong worked example builds nothing. Printed as the Telegram chat Carla would have had, then an 18-row beat ledger; exits `1` if any row reads `NOT as expected` | `python scripts/run_demo_scenario.py --data-dir .local_data/demo_scenario` | 0.1 s. Every model output is replayed from `src/repaso/simulator/cassettes/demo_fracciones.jsonl`, an authored cassette of 32 calls, so the run is byte-identical and costs nothing |
...
| `run_demo_scenario.py` | `.local_data/demo_scenario/` — the two notebook photographs it renders into `inbox/`, one family, one session, 3 grades, one resolved quarantine, the whole outbox the family would have received, and the telemetry of 32 replayed model calls | Nobody: this one is meant to be watched, not measured |
