...
(`run_seed_sweep.py`, 40 seeds by default) and at **20260950** (`run_sensitivity.py`), and the
gates those runs are scored against are frozen in `docs/reports/preregistered-gates-2026-08-20.md`
before the first run. Replay is exact: every simulated draw is
...
| `engagement alert never fires for active students` | `0` | `engagement - disengaged` is empty | An alert to a family doing nothing wrong. A school calendar (weekend, Carnaval, Semana Santa) would do it: silence is counted in calendar days |
