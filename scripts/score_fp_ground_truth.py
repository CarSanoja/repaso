        ...
        for student, dates in row["struggle_fires"].items():
            for raw in dates:
                yield (
                    row["seed"],
                    student,
                    row["archetypes"][student],
                    row["sections"][student],
...
def justified_cohort_fires(
    ...
    for row in results:
        ...
        for signal in row["cohort_signals"]:
            ...
            families = {
                student
                for held_section, student, fire in credited
...
def _print_per_archetype(per_archetype: dict[str, Counter]) -> None:
    ...
    for archetype in sorted(per_archetype, key=lambda a: -sum(per_archetype[a].values())):
        ...
        print(
            f"{archetype:16} {fires:>6} {counts[TRUE_POSITIVE]:>6} "
...
def _print_sensitivity(results: list[dict]) -> None:
    ...
    for criterion in CRITERIA:
        ...
        print(
            f"{criterion:>9.2f} {totals[MEASUREMENT_CORRECT]:>8} "
...
def main() -> int:
    ...
    print(f"  low-ability fires (TP):      {totals[TRUE_POSITIVE]}")
    print(f"  outside-archetype fires:     {outside}")
    print(f"    measurement-correct:       {totals[MEASUREMENT_CORRECT]}")
    print(f"    detector noise:            {totals[DETECTOR_NOISE]}")
