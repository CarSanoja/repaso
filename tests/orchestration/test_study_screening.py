...
async def test_an_injection_typed_into_a_sitting_never_becomes_an_attempt(settings):
    ...
    reply = await handle_study_text(services, family, INJECTION, 5.0, "m1")
...
async def test_an_injection_typed_into_a_sitting_is_quarantined_for_the_grown_up(settings):
    services = make_services(settings)
    family, student = seed_family(services.store)
    open_with(services, family, student)

    await handle_study_text(services, family, INJECTION, 5.0, "m1")
...
async def test_the_held_quote_is_redacted_before_it_is_stored(settings):
    services = make_services(settings)
    family, student = seed_family(services.store)
    open_with(services, family, student)

    await handle_study_text(services, family, f"{INJECTION} {WITH_A_PHONE}", 5.0, "m1")

    quote = services.store.list_quarantine(family.id)[0].evidence.quote
    assert "0414" not in quote


async def test_a_blocked_message_is_answered_and_the_question_stays_on_screen(settings):
    services = make_services(settings)
    family, student = seed_family(services.store)
    opened = open_with(services, family, student)
    item = current_item(services, opened.session)
...
async def test_a_blocked_message_does_not_spend_one_of_the_sitting_questions(settings):
    services = make_services(settings)
    family, student = seed_family(services.store)
    opened = open_with(services, family, student)
...
async def test_a_button_answer_is_not_sent_to_the_screener(settings):
    services = make_services(settings)
    family, student = seed_family(services.store)
    opened = open_with(services, family, student)
    item = current_item(services, opened.session)
    ...
    await handle_study_text(services, family, item.answer_key, 5.0, "m1")
